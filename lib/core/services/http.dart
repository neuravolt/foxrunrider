...
void _hydrateUserTokenFromStorage() {
  ...
  final raw = box.get("UserData");
  if (raw is! String || raw.isEmpty) return;
  try {
    final decoded = jsonDecode(raw);
    final storedToken = decoded["data"]?["token"]?.toString() ?? "";
...
Future<dynamic> httpPost(path, data, {required BuildContext context}) async {
  try {
    ...
    log(responseData.toString() + "post response data");
    if (response.statusCode == 498) {
      ...
      if (newToken != null) {
        ...
        log(response.body.toString() + "post response");
        responseData =
            json.decode(const Utf8Codec().decode(response.bodyBytes));
      } else {
...
Future<dynamic> httpGet(String path, Map<String, dynamic> data,
  ...
  try {
    ...
    log(response.body.toString() + "get response");
...
Future<String?> generateToken() async {
  ...
  try {
    ...
    log(response.body.toString() + "generate token response");
